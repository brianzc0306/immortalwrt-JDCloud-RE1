...
`oaf-test` 固件额外提供 **服务 → App Filter → 梦幻西游拦截（测试）** 页面。该功能默认关闭，仅当在页面中填写了目标设备的固定 IPv4 地址并启用后，才会下发 nftables 规则；页面可随时更换目标设备。保存并应用后会立即重载规则。它不会停止 Nikki，也不会修改 HNAT、ECM 或 Flow Offloading 设置。启用后应彻底退出并重新启动游戏，使新连接经过拦截规则。
